...
const ACTIVITY_AGE_GROUP_OPTIONS = [
  '0-6 month',
  '6-12 month',
  '12-24 month',
  '2-4 year',
...
class ActivityBenefitDto {
  ...
  @ApiProperty({
    example: 'Enhances cognitive development through problem-solving.',
  })
  @IsString()
  description: string;
...
class ActivityProgressionDto {
  ...
  @ApiProperty({
    example: 'For younger toddlers, start with larger, easy-to-find objects.',
  })
  @IsString()
  description: string;
}
...
export class CreateActivityDto {
  ...
  @ApiPropertyOptional({
    ...
    enum: ACTIVITY_AGE_GROUP_OPTIONS,
    example: '0-6 month',
  })
  @IsOptional()
  @IsIn(ACTIVITY_AGE_GROUP_OPTIONS)
  ageGroup?: string;
  ...
  @ApiPropertyOptional({
    ...
    isArray: true,
    example: [ActivityLocation.INDOOR, ActivityLocation.OUTDOOR],
  })
  @IsOptional()
  @IsArray()
  ...
  @ApiPropertyOptional({
    example:
      'Children co-create a shared imaginary world where ideas are visible, named, and valued by the group.',
  })
  @IsOptional()
  @IsString()
  ...
  @ApiPropertyOptional({
    example: 'Encourages curiosity and fine motor skill development.',
  })
  @IsOptional()
  @IsString()
  ...
  @ApiPropertyOptional({
    example: ['What color is this?', 'Can you find something red?'],
  })
  @IsOptional()
  @IsArray()
  @IsString({ each: true })
  ...
  @ApiPropertyOptional({
    example:
      'Ensure all small objects are safe for children and supervise to avoid choking hazards.',
  })
  @IsOptional()
  @IsString()
  safetyNotes?: string;
  ...
  @Transform(({ value }) => {
    if (typeof value === 'string') {
      try {
        ...
        return Array.isArray(parsed)
          ? parsed.map((item: any) => plainToInstance(ActivityBenefitDto, item))
          : parsed;
      } catch {
        return value;
      }
    ...
    if (Array.isArray(value)) {
      return value.map((item: any) =>
        plainToInstance(ActivityBenefitDto, item),
      );
    }
    return value;
  })
  @ApiPropertyOptional({
    type: 'string',
    description: 'JSON stringified array of benefits',
    example: `[
  ...
  @Transform(({ value }) => {
    if (typeof value === 'string') {
      try {
        const parsed = JSON.parse(value);
        return Array.isArray(parsed)
          ? parsed.map((item: any) => plainToInstance(ActivityStepDto, item))
          : parsed;
      } catch {
        return value;
      }
  ...
  @Transform(({ value }) => {
    if (typeof value === 'string') {
      try {
        ...
        return Array.isArray(parsed)
          ? parsed.map((item: any) =>
              plainToInstance(ActivityProgressionDto, item),
            )
          : parsed;
      } catch {
        return value;
  ...
  @ApiPropertyOptional({
    type: 'string',
    description: 'JSON stringified array of progressions',
    example: `[
